...
folderPath = [''];
fileName = ['20221022_ODbasedMOIresponse_LBM_run2.xlsx'];

%Data processing
timeVectorInfection = readmatrix([folderPath fileName], 'Sheet', 'Sheet2', 'Range', 'B37:J37')/60;
rawInfectionOD = readmatrix([folderPath fileName], 'Sheet', 'Sheet2', 'Range', 'B39:J86');
...
%Data processing
timeVectorDetection = readmatrix([folderPath fileName], 'Sheet', 'Sheet3', 'Range', 'B37:HX37')/60;
rawDetectionOD = readmatrix([folderPath fileName], 'Sheet', 'Sheet3',  'Range', 'B39:HX86');
...
conditionName = ["Infection of \lambda P+ in LBM at 30^oC, with optimal-physiology MG1655. Run 2."];

cellConcentration = [5.16e7]; %CFU/mL
cellVolume = 500; %uL
phageStockConcentration = 2.5e11; %PFU/mL
phageVolume = 10; %uL
MOIfolds = 2;
MOIseries = (phageStockConcentration .* MOIfolds.^(0:-1:-12) .* phageVolume) ./ (cellConcentration .* cellVolume);
% oneCellOD = 1/(2e9);

%Technical information for the TECAN plate
sampleReplicates = [repelem(3, 1, 12) 4 2 2 2 2];
...
for i_sample = 1:(nSamples-1)
    ...
    %(10/24) One reading replicate of infection sample #11 is an obvious outlier
    if i_sample == 11
        gVector(1) = [];
        initialVector(1) = [];
    end
